    ...
    const user = staff.find(
      (item) =>
        (item.username.toLowerCase() === loginValue ||
          item.staffId.toLowerCase() === loginValue) &&
...
function saveStaff() {
  ...
  const duplicate = staff.some(
    (person) =>
      ...
      (person.staffId.toLowerCase() === staffForm.staffId.trim().toLowerCase() ||
        person.username.toLowerCase() === staffForm.username.trim().toLowerCase())
  );
  ...
  if (editingStaff) {
    const updatedStaff = staff.map((person) =>
      person.id === editingStaff.id
        ? {
            ...
            role: staffForm.role,
            permissions: defaultPermissions[staffForm.role] || person.permissions || [],
            ...(staffForm.password
    ...
    const newStaff = {
      ...
      role: staffForm.role,
      status: "Active",
      permissions: defaultPermissions[staffForm.role] || [],
    };
